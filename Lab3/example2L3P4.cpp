...
int xt = 565;//origin x, top left corner
int yt = 140;//origin y, top left corner
...
//	int y4 = OldObject[7]+yt;//previous position of y + offset 140
	...
	rectangle(cameraFeed,Point(568,140),Point(897,570),Scalar(0,255,0),2);
...
int main() {
    ...
    while (frame_count < 1000) {
	   ...
	   //cv::cvtColor(frame1,grayImage1,cv::COLOR_BGR2GRAY);
           cv::cuda::cvtColor(gpu_frame1,gpu_grayImage1,cv::COLOR_BGR2GRAY);
           toggle = 1;
        } 
        else {
	   cap >> frame0;
	...
	if (toggle == 0) {
		//searchForMovement(thresholdImage,frame0);
                searchForMovement(result,frame0);
	        imshow("Frame", frame0);
	}
	else {
		//searchForMovement(thresholdImage,frame1);
                searchForMovement(result,frame1);
	        imshow("Frame", frame1);
	}
