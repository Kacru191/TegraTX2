	...
	if(m_mx<0){ //moving left
		...
		if(m_my<0){//moving up and left
			if(ty>tx){ //no bounce 
				...
				//m_xb = m_mx*tx + currentX; // calculates first impact 
				m_xb = m_xleft;
				m_yb = m_my*tx + currentY;
				//cout << "x: " << m_xb << ",y: " << m_yb << endl;

				cout << "UP/LEFT: NO HIT" << endl;

				m_rebounds[1] = cv::Point2i(m_xb,m_yb);

				m_goalPnt[0] = m_rebounds[1];	
				//return 0;
			}//end if	
			///////////////////////////////////////////////////////////////////////
				...
				yb1 = m_my*ty + currentY;

				ROIbound[0]=ROIbounds(xb1,yb1,1);

				m_xb = m_mx*ty + currentX; // calculates second impact 
				m_yb = m_my*ty + currentY;

		
				cout << "UP/LEFT: HITS" << endl;
				m_rebounds[1] = cv::Point2i(xb1,m_ytop);
				m_goalPnt[0] = 	cv::Point2i(m_xleft,m_yb);			
...
//////////////////////////////////////////////////////////////////////////////////////////		
			...
			//////////////////////////////////
				...
				m_xb = m_mx*ty1 + currentX;
				m_yb = m_my*ty1 + currentY;
		
				ROIbound[0]=ROIbounds(m_xb,m_yb,1);

				xb1 = m_mx*ty1 + currentX;
				yb1 = m_my*ty1 + currentY;
			
				m_rebounds[1] = cv::Point2i(m_xb,m_ybot);
				cout << "DOWN/LEFT: HITS" << endl;
				m_goalPnt[0] = cv::Point2i(m_xleft,yb1);		
			}//end hits bottom
	...
	if(m_mx>0){ //moving right
		if(m_my<0){//moving up and right
			if(ty < tx1){ // moving right hits top wall
				...
				ROIbound[0]=ROIbounds(xb1,yb1,0);		

				m_xb = m_mx*ty + currentX;
				m_yb = m_my*ty + currentY;
				
				m_rebounds[1] = cv::Point2i(xb1,m_ytop);
				cout << "UP/RIGHT: HITS" << endl;

				m_goalPnt[0] = cv::Point2f(m_xright,m_yb);
				//return 4;
			}
///////////////////////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////////////////
			...
			if(ty1 < tx1){ //moving right hits bottom wall 
				...
				ROIbound[1] = ROIbounds(m_xb,m_yb,0);

				m_rebounds[1] = cv::Point2i(xb1,m_ybot);//first bounce	
				cout << "DOWN/RIGHT: HITS" << endl;	
				m_goalPnt[0] = cv::Point2i(m_xright,m_yb);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////
cv::Point2i LeastSquares::ROIbounds(int p,int n,int direction){
