...
int xt = 330;//394;//x left 394
int yt = 208;//314;// y top 314
int xt2 = 520;//926; //x right 926
int yt2 = 700;//625; //y bottom 625
int xw = xt2-xt; //hidth
int yh = yt2-yt; //width
...
int xroi_L = 325;
int yroi_L = 314;
int xroi2_L =393;  // x coordinate of the opposite corner of the drawn rectangle
int yroi2_L = 625 ; // y coordinate of the oposite corner of the drawn rectangle 
...
int xroi_R = 928;
int yroi_R = 314;
int xroi2_R =980;  // x coordinate of the opposite corner of the drawn rectangle
int yroi2_R = 625 ; // y coordinate of the oposite corner of the drawn rectangle 
...
////////////////////////////////////////////////////////////////////////////////////////////
	...
	if(!LSF.testMonotonic())
		...
		line(cameraFeed, ImpactPnt[0],LSF.getGoal(),Scalar(255,0,0),2);//from bounce to paddle

		circle(cameraFeed,LSF.getGoal(),10,cv::Scalar(0,255,0),2);//the expected goal of ball
//////////////////////////////////////////////////////////////////////////////////////////
...
std::string pipeline = "nvcamerasrc auto-exposure=off flicker=auto tnr-mode=NoiseReduction_Fast ! video/x-raw(memory:NVMM), width=(int)1280, height=(int)720, format=(string)I420, framerate=(fraction)30/1 ! nvvidconv flip-method=0 ! video/x-raw, format=(string)BGRx ! videoconvert ! video/x-raw, format=(string)BGR ! appsink";
	...
	inputQuad[1] = Point2f(523,79);//523,79 
	inputQuad[2] = Point2f(880, 78);//880, 78
	inputQuad[3] = Point2f(924, 669); // 924,669 
	inputQuad[0] = Point2f(473, 655);//473,655
	
	// The 4 points where the mapping is to be done, from top-left in clockwise order
	outputQuad[0] = Point2f(300,233);//300,233
	outputQuad[1] = Point2f(1018,233);//1018,233
	outputQuad[2] = Point2f(1018,642);//1018,642
	outputQuad[3] = Point2f(300,642);//300,642
