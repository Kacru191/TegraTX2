...
int xt = 389;//x left 394
int yt = 321;// y top 314
int xt2 =935; //x right 926
...
int H_MIN_L = 0;//0;//24;
int H_MAX_L = 26;//25;//37;
int S_MIN_L = 67;//230;//26;
int S_MAX_L = 256;//255;//75;
int V_MIN_L = 69;//79;//225;
int V_MAX_L = 232;//103;//256;

int xroi_L = xt-30;//325;
int yroi_L = yt;//300;//314;
int xroi2_L = xt-1;//355;//393;  // x coordinate of the opposite corner of the drawn rectangle
...
void Move_Pot(){
		...
		if(Y_paddle > P_Max || Y_paddle < P_Min)
			Py = Py;
		else
			Py = Y_paddle;
		BYTE a;
...
//	Serial_Test(LSF.getGoal().y);
	...
	if (toggle == 0) {
		...
		/*char a = LSF.getGoal().y;
		char b = a;
		write_buffer[0] = '200';
		bytes_written = 0;
		cout << "Move Paddle to Position: "<< LSF.getGoal().y << endl;
		bytes_written = write(fd, &write_buffer, 1);


		//write_buffer[0] = LSF.getGoal().y;
		//bytes_written = 0;
